...
## Open Questions
> [!IMPORTANT]
> The `account_id` field in both the Invoice and Refund Definition tables requires selecting an Account. Since there can be many accounts, I plan to use the same **Select2** searchable dropdown pattern we used in `AccountManager`. Does that sound good?
